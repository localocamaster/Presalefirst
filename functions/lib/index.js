...
function getSmtpConfig() {
    ...
    if (!smtpUser || !smtpPass) {
        throw new inquiryErrors_1.SmtpConfigError({ message: "SMTP_USER or SMTP_PASS not configured" });
    }
    return { smtpHost, smtpPort, smtpUser, smtpPass, toEmail };
}
exports.sendInquiryEmail = functions.https.onRequest(async (req, res) => {
    ...
    const validation = (0, validation_1.validateInquiryPayload)(req.body);
    if (!validation.success) {
        const errorMessage = validation.error.field === "phone"
            ? "연락처는 필수입니다."
    ...
    `,
      ...
      `,
        ...
        (0, logger_1.logError)("Email send failed", err);
